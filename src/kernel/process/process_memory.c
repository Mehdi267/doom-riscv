...
int copy_process_memory(process* new_proc, process* old_proc){
  ...
  if (add_and_allocate_frames(new_proc, old_proc->mem_info.code_usage, CODE_PAGE, NULL)<0){return -1;}
  if (add_and_allocate_frames(new_proc, old_proc->mem_info.stack_usage, STACK_PAGE, NULL)<0){return -1;}
  if (add_and_allocate_frames(new_proc, old_proc->mem_info.heap_usage, HEAP_PAGE, NULL)<0){return -1;}
...
int add_and_allocate_frames(process* proc,  uint32_t page_nb,
  ...
  for (uint64_t page_iter = 0; page_iter < page_nb; page_iter++){
    ...
    if (frame.lvl0_index == 0 || 
      (type == STACK_PAGE && frame.lvl0_index == PT_SIZE - 1)){
